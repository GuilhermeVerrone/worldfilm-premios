  ...
  return (
    <div className="pb-24 md:pb-12">
      ...
      <div
        ...
        style={{ height: campanha.imagem_url ? undefined : undefined }}
      >
        <div className={cn('relative', campanha.imagem_url ? 'h-56 md:h-72 lg:h-80' : 'h-40 md:h-52')}>
          {campanha.imagem_url ? (
            <>
              <img
                src={campanha.imagem_url}
                alt={campanha.nome}
                className={cn('w-full h-full object-cover transition-opacity', encerrada && 'grayscale opacity-50')}
              />
              <button
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-3 p-8">
              ...
              <p className="text-white text-xl font-black uppercase tracking-wide text-center leading-snug">{campanha.nome}</p>
              <div className="w-12 h-0.5 bg-wf-red" />
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 px-4 pb-3 flex items-end justify-between gap-2">
            ...
            {!encerrada && (
              <div className={cn('shrink-0 px-3 py-2 text-center', urgent ? 'bg-wf-red' : 'bg-black/60')}>
                ...
                <p className="text-white font-black text-xl leading-tight">{days === 0 ? 'Hoje' : `${days}d`}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          {/* Left column: prizes + description */}
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isTop ? 'bg-[#111827] border-transparent' : 'bg-white border-wf-border',
                        )}
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <p className={cn('font-bold truncate', isTop ? 'text-white text-base' : 'text-wf-text-primary text-sm')}>
                        ...
                        <p className={cn('font-black shrink-0 ml-4', isTop ? 'text-green-400 text-2xl' : 'text-green-600 text-xl')}>
          ...
          <div className="space-y-4">
            ...
            {!encerrada && (
              <button
                ...
                className="hidden lg:block w-full py-4 bg-wf-red hover:bg-wf-red-dark text-white text-xs font-black uppercase tracking-widest transition-colors"
              >
                Registrar venda
              </button>
            )}
      ...
      {!encerrada && (
        <div className="lg:hidden fixed bottom-16 md:bottom-0 left-0 right-0 md:left-60 p-3 bg-white border-t border-wf-border z-20">
          <button
            onClick={() => navigate(`/vendedor/venda/nova?campanha_id=${id}`)}
            className="w-full max-w-lg mx-auto block py-4 bg-wf-red hover:bg-wf-red-dark text-white text-xs font-black uppercase tracking-widest transition-colors"
          >
