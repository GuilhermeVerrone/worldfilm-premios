...
export default function VendedorCampanhas() {
  ...
  return (
    <div>
      {/* ── Header ── */}
      <div className="bg-[#0A0A0A] px-4 pt-5 pb-6">
          ...
          filtered.map((c: any) => {
            ...
            return (
              <button
                ...
                className="w-full text-left overflow-hidden rounded-xl shadow-sm bg-white active:scale-[0.99] transition-transform shadow-sm"
              >
                {/* ── Image area ── */}
                <div className="relative h-48 bg-[#0A0A0A] overflow-hidden">
                  {c.imagem_url ? (
                    ...
                    /* Fallback: branded dark tile with name */
                    <div className="w-full h-full flex flex-col items-center justify-center p-6 gap-3">
                      <div className="w-10 h-0.5 bg-wf-red" />
                      <p className="text-white text-lg font-black uppercase tracking-wide text-center leading-snug">
                        {c.nome}
                      </p>
                      <div className="w-10 h-0.5 bg-wf-red" />
                    </div>
